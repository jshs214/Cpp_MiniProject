...
void ShopInfoManager::search_shoplist()    //검색
{
	...
	case 2:
		...
		for (auto it = ShopInfolist.begin(); it != ShopInfolist.end(); ++it)
		{
			if ((*it)->getProductID().find(input) != -1)	// 입력한 input이 ProductID에 포함되면
			{
				string p_ID = (*it)->getProductID();	//p_ID 변수에 ShopInfolist의 ProductID 저장
				string c_ID = (*it)->getclientID();		//c_ID 변수에 ShopInfolist의 ClientID 저장
				for (auto pit = PM.getproductList().begin(); pit != PM.getproductList().end(); ++pit)
				{
					if ((*pit)->getProductID().find(input) != -1) {	// 클라이언트 ID와 입력한 input 문자열에 겹치는게 있으면
						flag = true;
						if (p_ID == (*pit)->getProductID()) {	//  ShopInfolist 벡터와 Product벡터에 있는 Productid가 같으면 Product에 있는 벡터 데이터 값 복사
							productname = (*pit)->getProductName();
							producttype = (*pit)->getProductType();
							price = (*pit)->getPrice();
							sum = price * (*it)->getStock();
...
void ShopInfoManager::update_shop()	//쇼핑리스트 변경 함수
{
	int input;	// 변경 시 주문번호가 있는지 검사하기위한 변수
	int up_stock;	// 변경 시 업데이트할 수량 변수
	string up_data;	// 변경 시 업데이트할 주소, 연락처 변수
	int num;	// 재고, 연락처, 배송지 변경위한 swtich case 변수
	bool flag = false;
	...
	case 1:	// 재고변경
		...
		for (auto it = ShopInfolist.begin(); it != ShopInfolist.end(); ++it)    //Shoplist 벡터 출력
		{
			if ((*it)->getShopkey() == input) {		//ShopInfo PK키와 입력값이 같으면
				for (auto pit = PM.getproductList().begin(); pit != PM.getproductList().end(); ++pit) {
					auto sid = (*it)->getProductID();	//shoplist에 있는 제품코드
					auto pid = (*pit)->getProductID();	//pid product 기본키를 가져옴
					if (sid == pid) {	//가져온 ShopInfo의 제품코드를 Product에있는 제품코드와 매칭
						flag = true;
						cout << "변경 수량 입력 : ";	cin >> up_stock;	// 변경할 수량 up_inputstock
						if (up_stock <= 0) {
							...
							(*it)->setStock(up_stock);		//Shoplist 주문수량 변경
...
int ShopInfoManager::ssearach_menu()		// 메뉴 입력 예외처리
{
	int menu;
	cin >> menu;
	if (!cin) {		// cin menu 에 숫자만 입력 받도록
